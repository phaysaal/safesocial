...
//!
//! Each Spheres user has a single identity keypair stored in the Veilid
    ...
    match (secret_str, public_str) {
        (Some(secret_b64), Some(public_b64)) => {
            let secret_bytes = BASE64.decode(&secret_b64).map_err(|e| {
                crate::SpheresError::CryptoError(format!("Failed to decode secret key: {}", e))
            })?;
            let public_bytes = BASE64.decode(&public_b64).map_err(|e| {
                crate::SpheresError::CryptoError(format!("Failed to decode public key: {}", e))
            })?;
...
pub fn identity_from_string(s: &str) -> Result<KeyPair> {
    let bytes = BASE64.decode(s).map_err(|e| {
        crate::SpheresError::CryptoError(format!("Failed to decode identity string: {}", e))
    })?;

    let expected_len = 4 + KEY_LENGTH * 2;
    if bytes.len() != expected_len {
        return Err(crate::SpheresError::CryptoError(format!(
