...
class BackupService {
  ...
  Future<String> createBackup({String? passphrase}) async {
    ...
    // Gather all SafeSocial data from SharedPreferences
    final backupData = <String, dynamic>{
      'version': _backupVersion,
      'created_at': DateTime.now().toIso8601String(),
      'identity': {
        'keypair': prefs.getString('safesocial_identity_keypair'),
        'profile': prefs.getString('safesocial_identity_profile'),
        'dht_key': prefs.getString('safesocial_identity_dht_key'),
      },
      'contacts': prefs.getString('safesocial_contacts'),
      'conversations': prefs.getString('safesocial_conversations'),
      'feed_posts': prefs.getString('safesocial_feed_posts'),
      'hidden_posts': prefs.getStringList('safesocial_hidden_posts'),
      'theme_mode': prefs.getString('theme_mode'),
    };

    // Also gather cached messages
    final msgKeys = prefs.getKeys().where((k) => k.startsWith('safesocial_msgs_'));
    ...
    final filePath = '${dir.path}/safesocial_backup_$timestamp.ssb';
  ...
  Future<void> restoreBackup(String filePath, {String? passphrase}) async {
    ...
    if (identity != null) {
      if (identity['keypair'] != null) {
        await prefs.setString('safesocial_identity_keypair', identity['keypair'] as String);
      }
      if (identity['profile'] != null) {
        await prefs.setString('safesocial_identity_profile', identity['profile'] as String);
      }
      if (identity['dht_key'] != null) {
        await prefs.setString('safesocial_identity_dht_key', identity['dht_key'] as String);
      }
    }

    // Restore contacts
    if (backupData['contacts'] != null) {
      await prefs.setString('safesocial_contacts', backupData['contacts'] as String);
    }

    // Restore conversations
    if (backupData['conversations'] != null) {
      await prefs.setString('safesocial_conversations', backupData['conversations'] as String);
    }

    // Restore feed posts
    if (backupData['feed_posts'] != null) {
      await prefs.setString('safesocial_feed_posts', backupData['feed_posts'] as String);
    }
    ...
    if (backupData['friend_requests'] != null) {
      await prefs.setString('safesocial_friend_requests', backupData['friend_requests'] as String);
    }
